...
prior_for <- function(.vars, cor = TRUE) {
  if (cor) {
    c(set_prior("normal(0, 3)", class = "b", resp = .vars)
      , set_prior("normal(1, 3)", class = "sd", resp = .vars, lb = 0)
      , set_prior("normal(1, 3)", class = "sigma", resp = .vars, lb = 0)
